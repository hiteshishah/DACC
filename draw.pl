...
:- drucke_baum(s(np(adj(large), n(crowds)), vp(v(listened), pp(p(to), np(det(the), n(orchestra)))))).
:- drucke_baum(s(np(det(the), n(man)), vp(v(rejected), np(adj(elective), n(surgery))))).
:- drucke_baum(s(np(det(an), n(airplane)), vp(v(flew), pp(p(over), np(det(the), n(city)))))).
